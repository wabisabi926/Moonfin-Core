...
// since the libretro host uses global C callbacks.
public final class NativeGameChannel: NSObject, FlutterStreamHandler {
  ...
  public init(registrar: FlutterPluginRegistrar) {
    // The registrar exposes these as properties on macOS and as methods on iOS.
    #if canImport(FlutterMacOS)
    ...
    control = FlutterMethodChannel(
      name: "moonfin/native_game_control", binaryMessenger: messenger)
    events = FlutterEventChannel(
      name: "moonfin/native_game_events", binaryMessenger: messenger)
  ...
  private func load(_ args: [String: Any], result: @escaping FlutterResult) {
    ...
    // The bundled cores come from the app rather than Dart.
